...
def train_moe_model_numpy(
    ...
    with Timer() as timer:
        moe_models["MoE-global-int"] = moe_global_int.fit()
...
def train_all_models(
    ...
    if not skip_baseline:
        baseline_models, baseline_runtimes = train_baseline_linear_models(
            dataset, **baseline_kwargs
        )
        trained_models.update(baseline_models)
        runtimes.update(baseline_runtimes)

    if not skip_moe:
        moe_models, moe_runtimes = train_moe_model_numpy(dataset, **moe_kwargs)
        trained_models.update(moe_models)
        runtimes.update(moe_runtimes)
